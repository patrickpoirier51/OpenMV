...
valid_tag_ids = {
                  ...
                  4 : 165, # 8.5" x 11" tag black border size in mm
                }
...
#sensor.set_windowing((180, 150)) # Look at center 160x120 pixels of the VGA resolution.
...
c_x = x_res / 2
c_y = y_res / 2
h_fov = 2 * math.atan((sensor_w_mm / 3.5) / lens_mm)
v_fov = 2 * math.atan((sensor_h_mm / 2.5) / lens_mm)

def z_to_mm(z_translation, tag_size): # z_translation is in decimeters...
    return ((((z_translation * 100) * tag_size) / 559) - lens_to_camera_mm)*-1
...
MAV_DISTANCE_SENSOR_min_distance = 30 # in cm
MAV_DISTANCE_SENSOR_max_distance = 200 # in cm
...
#!!!!!!!!!!!!!!PAPOU MOD === REDUCED RANGE TAG / ADJUST & ORIENTATION  !!!!!!!!!!!!!
def send_landing_target_packet(tag, w, h, tag_size):
    global packet_sequence
    temp = struct.pack("<qfffffbb",
                       0,
                       (((tag.cx() / w) - 0.5) * h_fov)/.8,
                       (((tag.cy() / h) - 0.5) * v_fov)/.8,
                       int(z_to_mm(tag.z_translation(), tag_size) / -10),
...
while(True):
    ...
    if tags and (tags[0].id() in valid_tag_ids):
        ...
        print("TAG %d - X %d  Y %d " % (tags[0].id(),tags[0].cx(), tags[0].cy()))
